    ...
    try{
        ...
        try{
            ...
            if(!(opts && opts.noColorKey)){
                // Determine background color to strip.
                // If caller provided opts.forceBgColor (e.g. [255,255,255]) use that,
                // otherwise fall back to top-left pixel as before.
                let id;
                if(opts && Array.isArray(opts.forceBgColor) && opts.forceBgColor.length >= 3){
                    id = opts.forceBgColor;
                } else {
                    id = octx.getImageData(0,0,1,1).data; // top-left pixel as background
                }
...
// Load bee sprite and strip white background if present. Use
// forceBgColor to target white; set tolerance for near-white pixels.
try{ loadMobSprite('bee', 'assets/bee.png', { forceBgColor: [255,255,255], tolerance: 24 }); }catch(e){}

const BEE_ROT_OFFSET = Math.PI / 2; // 90° rotation offset; adjust if needed
